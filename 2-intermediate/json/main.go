...
type Person struct {
	...
	Age          int     `json:"age,omitempty"` // omit age field on the json output if age not filled
...
func main() {
	person := Person{FirstName: "John", EmailAddress: "sample@example.com"}
	...
	if err != nil {
		log.Fatalln("Errror Unmarshalling JSON:", err)
	}
