...
func main() {
	...
	fmt.Println("Join Path:", joinedPath)
	...
	fmt.Println("is relativePath abs?", filepath.IsAbs(relativePath))
	fmt.Println("is absolutePath abs?", filepath.IsAbs(absolutePath))
	...
	absPath2, err := filepath.Abs(relativePath2)
	if err != nil {
		fmt.Println("Error:", err)
	} else {
		fmt.Println("Absolute Path:", absPath2)
	}
}
